...
def login():
    ...
    if st.button("Log In"):
        ...
        if username in credentials and credentials[username]["password"] == hashed_password:
            ...
            st.session_state["user_id"] = credentials[username]["id"]
            st.rerun()  
        else:
...
def save_ratings(new_ratings):
    try:
        ...
        combined_ratings = existing_ratings + new_ratings
        unique_ratings = {f"{r['user_id']}_{r['movie_title']}": r for r in combined_ratings}.values()
...
def main():
    if "df" not in st.session_state:
        ...
        if not saved_ratings.empty:
            saved_ratings = saved_ratings.rename(columns={
                "user_id": "userId",
                "movie_title": "title"
            })
            merged_df = pd.merge(
                saved_ratings,
                original_df[['title', 'movieId', 'genres']].drop_duplicates(),
                how='left',
                on='title'
            )
            st.session_state.df = pd.concat([original_df, merged_df], ignore_index=True)
        else:
    ...
    with tab3:
        ...
        user_id = st.number_input("Enter Your User ID:", min_value=1, value=int(st.session_state["user_id"]))
        ...
        if user_id not in ratings_matrix.index:
            ...
            similar_users = similarity_df[user_id].sort_values(ascending=False).iloc[1:6]
            st.subheader("Top 5 Users with Similar Preferences")
            st.dataframe(similar_users.reset_index().rename(columns={user_id: "Similarity"}), use_container_width=True)
    ...
    with tab5:
        ...
        user_id = st.number_input("Enter User ID", min_value=1, value=int(st.session_state["user_id"]))
        ...
        if not user_ratings.empty:
            ...
            with col4:
                st.metric("Years Span", f"{int(yearly_ratings['release_year'].max() - yearly_ratings['release_year'].min())}")
        else:
